...
async fn main() -> anyhow::Result<()> {
    ...
    // Use spiffe-rustls-tokio::TlsAcceptor which automatically extracts peer identity
        ...
        tokio::select! {
            res = listener.accept() => {
                ...
                tokio::spawn(async move {
                    // Use spiffe-rustls-tokio::TlsAcceptor which returns (TlsStream, PeerIdentity)
                    match acceptor.accept(tcp).await {
                        Ok((mut tls, peer_identity)) => {
                            // Extract and display peer SPIFFE ID
                            if let Some(spiffe_id) = peer_identity.spiffe_id() {
                                println!("TLS handshake OK from {peer} (SPIFFE ID: {spiffe_id})");
                            } else {
                                println!("TLS handshake OK from {peer} (no SPIFFE ID)");
                            }
                        ...
                        Err(e) => eprintln!("TLS connection failed from {peer}: {e}"),
                    }
                });
            }
